...
app.get('/deploy', function(request, response){
  // mySocket.emit('testdata', {some: 'Hello World'}, function acknowledgement_callback (result) {
  //   console.log("we are  emitting inside DEPLOY route");
  //   console.log(result)
  // });
...
app.post('/migrate', function(request, response){
  ...
  mag.image=request.body.image;
  mag.port=request.body.port;
  ...
  mySocket.emit("migrate_source@"+ mag.ip+"", mag, function acknowledgement_callback (result) {
    ...
    // var exec=require('child_process').exec;
    //
    // var command="./pre_migrate.sh " + mag.source + " " + mag.application + " " + mag.image+" " + mag.port + " " + mag.destination;
    // exec(command,function(err, stdout){
    //     if(err){
    //       throw err;
    //     }
